...
private_lane :smf_verify_git_tag_is_not_already_existing do |options|
  ...
  if git_tag_exists(tag: git_tag) or git_tag_exists(tag: deprecated_git_tag)
    raise "The Git tag already exists! The build will be aborted to avoid builds with the same build nubmer. If this surprises you: It may be a good time to get some help!"
  end
